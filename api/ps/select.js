...
export default async function handler(req, res) {
  ...
  console.log("select query ", ip, teamCode);
  ...
  // ── Authentication ──────────────────────────────────────────────────────────
  // The Team Code itself serves as the secret/password for the PS Drop.
  // Rate limiting prevents brute force guessing of the UDB-XXXX code.
  ...
  // ── Database Operations (No Transaction to support standalone MongoDB) ──
  await connectDB();

  try {
    let result = null;

    // 1. Fetch the team row
    ...
    if (team.psSelectionId) {
      const existingPS = await ProblemStatement.findById(team.psSelectionId).lean();
    ...
    // 3. Atomically increment slotsTaken if available
    ...
    if (!ps) {
      ...
      const psCheck = await ProblemStatement.findOne({ order: psOrder }).lean();
      if (!psCheck) {
    ...
    await Team.updateOne(
      { _id: team._id },
      { $set: { psSelectionId: ps._id, psSelectedAt: selectedAt } }
    );
